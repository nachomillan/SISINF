...
router.post('/agnadirPeliALista', async (req, res) => {
  try {
    ...
    console.log(idLista, idProd)
    const validarAdicion = await FuncionesListas. existePeliEnLista(idLista, idProd)
    if(validarAdicion){
      res.status(404).json("Esta peli ya se encuentra en la lista")
    }else{
